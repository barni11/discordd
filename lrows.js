...
fs.readdir("./commands/", (err, files) => {
  ...
  log(`${files.length} commands will load.`);
  files.forEach(f => {
...
lrows.on('message', async msg =>{
  ...
  let categories = lrows.gif.category
  ...
  msg.reply('You can only share pp/gif on this channel!').then(m=>m.delete({timeout:2000}))
...
lrows.login(ayarlar.token).catch(err=> console.error('Not a good token bud.'));
